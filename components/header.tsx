...
const navTriggerClass =
  "px-3 py-2 text-xs font-bold uppercase tracking-wider transition-colors text-deep-navy bg-transparent hover:bg-transparent hover:text-electric-blue data-[state=open]:bg-transparent data-[state=open]:text-electric-blue focus:bg-transparent focus-visible:ring-0 h-auto shadow-none"

function DesktopNav() {
  return (
    <NavigationMenu
      viewport={false}
      className="max-w-none [&_[data-slot=navigation-menu-content]]:!bg-white [&_[data-slot=navigation-menu-content]]:!text-deep-navy [&_[data-slot=navigation-menu-trigger]]:bg-transparent [&_[data-slot=navigation-menu-trigger]]:hover:bg-transparent [&_[data-slot=navigation-menu-trigger]]:data-[state=open]:bg-transparent [&_[data-slot=navigation-menu-link]]:bg-transparent [&_[data-slot=navigation-menu-link]]:hover:bg-transparent [&_[data-slot=navigation-menu-link]]:focus:bg-transparent"
    >
          ...
          entry.type === "link" ? (
            <NavigationMenuItem key={entry.href}>
              <NavigationMenuLink asChild>
                <Link href={entry.href} className={navTriggerClass}>
                  {entry.label}
                </Link>
              </NavigationMenuLink>
            </NavigationMenuItem>
                  ...
                  {entry.items.map((item) => (
                    <li key={item.href}>
                      <NavigationMenuLink asChild>
                        <Link
                          href={item.href}
                          className="block rounded-lg px-3 py-2.5 text-sm font-bold text-deep-navy uppercase tracking-wide hover:bg-transparent hover:text-electric-blue transition-colors focus:bg-transparent"
                        >
                          {item.label}
                        </Link>
                      </NavigationMenuLink>
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className="bg-white border border-slate-100 border-l-0 rounded-br-2xl lg:rounded-br-3xl shadow-lg px-3 py-2.5 pb-4 sm:px-4 sm:py-3 sm:pb-5 lg:px-5 lg:py-4 lg:pb-6">
            <Image
              src="/images/qk-logo.png"
              alt="QK Coldstores"
              width={448}
              height={224}
              className="h-14 sm:h-16 lg:h-24 w-auto object-contain block"
              priority
            />
          </div>
        </Link>
        ...
        <div className="pointer-events-auto relative z-[1001] bg-white border-b border-slate-100 shadow-sm">
          <nav className="container mx-auto px-4 sm:px-6 lg:px-8 flex h-14 lg:h-16 items-center justify-between gap-2">
            <div className="hidden lg:block w-[11rem] shrink-0" aria-hidden="true" />
